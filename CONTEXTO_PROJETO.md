...
> - ✅ **2026-08-27**: mesclado em `main`/produção; campo `superintendencia` (text, NORTE/NORDESTE/SUL/SUDOESTE/CENTRO) adicionado em `d_subestacoes` e retropreenchido nas 369 subestações (`sql_add_superintendencia_subestacoes.sql`) — faltava na modelagem original, é o campo "SUPERINTENDÊNCIA" da planilha; campo dinâmico `equipe_regional` (select, 5 opções fixas: AT - SUD 04/CENTRO/NORDESTE/NORTE/SUL) criado no **lançamento** (`config_campos`/`config_campos_contrato`, seção "registro", igual ao campo "OS") para a coluna "EQUIPE" da planilha original — tentativa inicial colocou como atributo fixo em `d_subestacoes`, corrigida no mesmo dia após o histórico mostrar que EQUIPE varia por visita, não por subestação (`sql_equipe_regional_campo_lancamento.sql`); auto-filtro de Atividade por porte/tipo da subestação no lançamento; relatório de exportação "Limpeza de Subestação" em `/relatorios/exportacao`, no formato da planilha original (1 linha por visita concluída, reconstruída a partir dos pares Em Andamento + conclusão); colunas `regional_id`/`equipe_interna_id` removidas de `d_subestacoes` (nunca usadas de fato) e da tela de cadastro.
